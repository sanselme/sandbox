...
extension Matter {
  class Node {
    ...
    init() {
      ...
      innerNode = RootNode(
        attribute: self.eventHandler, identify: { _, _, _, _ in self.identifyHandler?() })!
    }

    func eventHandler(
      type: MatterAttributeEvent, endpoint: __idf_main.Endpoint, cluster: Cluster,
      attribute: UInt32, value: UnsafeMutablePointer<esp_matter_attr_val_t>?
    ) {
    ...
    enum Attribute {
      ...
      init?(cluster: Cluster, attribute: UInt32) {
        if cluster.as(OnOff.self) != nil {
          ...
          switch attribute {
          case LevelControl.AttributeID<LevelControl.CurrentLevel>.currentLevel.rawValue:
            self = .levelControl
          default: return nil
          }
        } else if cluster.as(ColorControl.self) != nil {
          switch attribute {
          case ColorControl.AttributeID<ColorControl.CurrentHue>.currentHue.rawValue:
            self = .colorControl(.currentHue)
          case ColorControl.AttributeID<ColorControl.CurrentSaturation>.currentSaturation.rawValue:
          ...
          case ColorControl.AttributeID<ColorControl.ColorTemperatureMireds>.colorTemperatureMireds
            .rawValue:
            self = .colorControl(.colorTemperatureMireds)
          case ColorControl.AttributeID<ColorControl.ColorMode>.colorMode.rawValue:
